...
public sealed class AgentRelayRuntimeTests
{
    ...
    [Fact]
    public async Task Runtime_acknowledges_events_only_after_projection_returns()
    {
        var requests = new List<ProtocolEnvelope>();
        var applied = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var acknowledged = new TaskCompletionSource(TaskCreationOptions.RunContinuationsAsynchronously);
        var eventEnvelope = ProtocolEnvelope.Create("event-1", "agent_event",
            new AgentEventMessage("codex", "source-1", "task-1", 1, "task_started", DateTimeOffset.UtcNow));
        var sentEvent = 0;
        var client = new AgentControlClient((request, _) =>
        {
            lock (requests) requests.Add(request);
            if (request.MessageType == "event_ack") acknowledged.TrySetResult();
            object payload = request.MessageType switch
            {
                "connection_test" => new RelayConnectionTestResponse(true, true, false, "1", "adapter_offline", DateTimeOffset.UtcNow, null),
                "pending_sources" or "list_sources" => new { result = request.MessageType, sources = Array.Empty<object>() },
                "event_ack" => new { result = "acknowledged" },
                "status_check" when request.Payload.TryGetProperty("include_events", out var include) && include.GetBoolean()
                    && Interlocked.Exchange(ref sentEvent, 1) == 0 => new { result = "status", events = new[] { eventEnvelope } },
                _ => new { result = "status", events = Array.Empty<object>() },
            };
            return Task.FromResult(ProtocolEnvelope.Create(request.MessageId, request.MessageType, payload).ToJson());
        });
        using var runtime = new AgentRelayRuntime(new AgentRelayClient(client), new AgentRelayAdministration(client),
            ...
            (_, _) =>
            {
                lock (requests) Assert.DoesNotContain(requests, item => item.MessageType == "event_ack");
    ...
    [Fact]
    public async Task Gateway_chunks_event_acknowledgements_at_protocol_limit()
    {
        var requests = new List<ProtocolEnvelope>();
        var client = new AgentControlClient((request, _) =>
        {
        ...
        var gateway = new AgentRelayClient(client);
        var events = Enumerable.Range(1, 513)
            .Select(sequence => new AgentEvent("codex", "source-1", "task-1", sequence,
                AgentEventType.TaskUpdated, DateTimeOffset.UtcNow))
            .ToArray();

        await gateway.AcknowledgeEventsAsync(events);

        var acknowledgements = requests.Where(item => item.MessageType == "event_ack").ToArray();
        Assert.Equal(2, acknowledgements.Length);
        Assert.Equal(512, acknowledgements[0].Payload.GetProperty("event_keys").GetArrayLength());
        Assert.Equal(1, acknowledgements[1].Payload.GetProperty("event_keys").GetArrayLength());
    }
