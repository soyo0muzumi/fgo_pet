...
public sealed class RelayResponseContractTests
{
    ...
    public void Runtime_acknowledgements_and_errors_are_responses_only()
    {
        var responses = new[]
        {
            ...
            ProtocolEnvelope.Create("i", "event_ack", new { result = "acknowledged" }),
            ProtocolEnvelope.Create("j", "dispatch_ack", new { result = "already_acknowledged" }),
    ...
    [Fact]
    public void Acknowledgement_requests_are_bounded_and_reject_null_keys()
    {
        var valid = ProtocolEnvelope.Create("ack", "event_ack", new EventAcknowledgementRequest(
            "codex", "source-1", new[] { new EventAcknowledgement("task-1", 1) }));
        AgentProtocolValidator.Validate(valid);
        AgentProtocolValidator.ValidateResponse(ProtocolEnvelope.Create("ack-r", "event_ack", new { result = "acknowledged" }));

        var nullKey = ProtocolEnvelope.Create("bad", "event_ack", new EventAcknowledgementRequest(
            "codex", "source-1", new EventAcknowledgement[] { null! }));
        Assert.Throws<AgentProtocolValidationException>(() => AgentProtocolValidator.Validate(nullKey));
    }
