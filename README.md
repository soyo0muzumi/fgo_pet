...
- **Phase 4 Agent integration (machine-complete, manual matrix pending).** TODO items can be proposed and confirmed, dispatched through a private versioned protocol and local relay, projected into a bounded Todo timeline, and archived into user-confirmed work summaries. The Codex adapter is packaged as a local plugin with App Server, hook, and MCP boundaries; credentials, source allowlists, deduplication, privacy redaction, reconnect recovery, export, and clear-data flows are covered by tests. Exact Codex desktop navigation remains capability-gated and falls back to a safe in-app task view when the runtime does not prove support. Handoff: `docs/reports/2026-08-30-phase4-agent-integration-handoff.md`.
- **Still unavailable:** LLM/Prompt/memory automation remains limited to explicit user-confirmed flows; Codex/Agent manual desktop validation remains to be run on a real Windows installation.
