...
/// </summary>
public sealed partial class AttachedPanelViewModel : ObservableObject, IAttachedPanelLauncher
{
    ...
    // Compact timer surface —— 任务卡 C 步骤 3+4：这里不再存 focus 的派生状态。
    //   可见性：现算自权威源 Current，不存布尔镜像；
    //   三个展示串：交给 View 层的 FocusDisplayText 格式化，中文字面量既不进 shell VM，
    //              更不进 focus 领域模型（FocusSession）。
    // 只有 ProgressPercent 仍需保留字段：它是唯一需要"数值原样"的（View 拿它画进度弧）。
    ...
    private void OnFocusChanged()
    {
        ...
        // 只在"可见性真的翻转"时才通知。SnapshotChanged 每秒都来一次，而 View 与
        // PortraitWindowCoordinator 收到这个通知会重排整个面板 —— 每秒重排一次既浪费，
        // 也会让 portrait 抖。值本身不落字段，一律现算（任务卡 C 步骤 3）。
        var surfaceWasVisible = IsCompactTimerSurfaceVisible(_lastObservedStatus);
        _lastObservedStatus = session.Status;
        if (surfaceWasVisible != IsCompactTimerSurfaceVisible(session.Status))
        {
            OnPropertyChanged(nameof(IsCompactTimerVisible));
        }

        // 倒计时要每秒刷新，所以这条通知不做去重 —— 它本来就该每秒来一次。
