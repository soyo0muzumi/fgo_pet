...
public sealed record CodexDispatchRecord(DispatchTaskRequest Request, long Sequence, string State,
    string? ThreadId = null, AgentEventMessage? PendingEvent = null);
...
public sealed class CodexDispatchWorker
{
            ...
            try
            {
                ...
                if (connection.Status == AdapterConnectionStatus.Connected)
                {
                    ...
                    foreach (var request in requests)
                    {
                        ...
                        if (_records.ContainsKey(request.DispatchRequestId)) continue;
                        if (_records.Count >= 512) throw new InvalidDataException("dispatch_journal_full");
    ...
    private CodexDispatchRecord SetEvent(CodexDispatchRecord record, string type, string summary, bool terminal)
    {
        ...
        var updated = record with { Sequence = message.Sequence, State = terminal ? "terminal" : record.State, PendingEvent = message };
        Save(updated);
        return updated;
    }
