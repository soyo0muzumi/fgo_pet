...
当前证据文件中的样本均为 `pending` 候选，只能保守参考有来源的称呼和语言风格；剧情事实、关系或成长结论的注入必须等待 `approved`。候选短引显示了“前辈/先輩”称呼，以及在明确台词出现时的保护/守护、职责/责任、安全关切、鼓励、感谢、道歉或当下情绪词。称呼不等于亲密关系；单个战斗关键词不等于稳定责任感。没有已审核证据时，不主动补写剧情设定。
