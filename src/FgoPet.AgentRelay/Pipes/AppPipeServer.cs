...
public sealed class AppPipeServer
{
    ...
    private Task<string> ProcessLineCoreAsync(string line, DateTimeOffset at, bool consume = true)
    {
        ...
        switch (envelope.MessageType)
        {
            ...
            case "event_ack":
            {
                var request = envelope.DeserializePayload<EventAcknowledgementRequest>();
                var result = _router.AcknowledgeInbound(request, at);
                return Task.FromResult(Response(messageId, "event_ack", new { result }).ToJson());
            }
