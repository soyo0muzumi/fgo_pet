...
public sealed class AgentRelayClient : IAgentGateway
{
    ...
    public async Task<AgentDispatchResult> DispatchAsync(AgentDispatchRequest request, CancellationToken cancellationToken = default)
    {
        ...
        return new AgentDispatchResult(
            ...
            request.DispatchRequestId,
            result is "accepted" or "alreadyapplied" or "already_applied" ? null
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var group in events.GroupBy(item => (item.SourceType, item.SourceInstance)))
        {
            foreach (var chunk in group.Select(item => new EventAcknowledgement(item.TaskId, item.Sequence)).Chunk(512))
            {
                var request = new EventAcknowledgementRequest(group.Key.SourceType, group.Key.SourceInstance, chunk);
                var envelope = ProtocolEnvelope.Create(
                    "event-ack-" + Guid.NewGuid().ToString("N"), "event_ack", request);
                AgentProtocolValidator.Validate(envelope);
                var response = await SendAsync(envelope, cancellationToken).ConfigureAwait(false);
                if (response is null)
                    throw new AgentRelayException("relay_offline");
                var result = response.Payload.TryGetProperty("result", out var value) && value.ValueKind == JsonValueKind.String
                    ? value.GetString()
                    : null;
                if (result is not "acknowledged" and not "already_acknowledged")
                    throw new AgentRelayException("event_ack_rejected");
            }
        }
    }
