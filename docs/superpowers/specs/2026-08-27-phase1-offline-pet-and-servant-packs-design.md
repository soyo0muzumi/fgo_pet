...
Phase 1 交付一个不依赖 Python、LLM 或 Codex 插件即可独立运行的 Windows 11 桌宠宿主。首发版本随应用安装玛修默认扩展包，同时建立可持续扩展的纯资源 `.fgopetpack` 协议，使后续新增从者、外观、表情和人格资源时不需要修改应用代码。
...
- 内置玛修首发包与本地从者扩展包安装、校验、卸载、重扫、版本共存和回退。
- 从者、外观、比例和置顶设置。
- 独立从者库与设置窗口。
...
安装失败时删除 staging 并保留当前包。更新不会覆盖旧版本。卸载当前包前必须先切换到其他有效包。内置玛修包标记为官方内置且不可卸载；第三方无签名包允许安装，但始终显示“未验证来源”。包内自称 `verified` 不影响应用信任判断。

GitHub Releases 是首期分发渠道。Release 提供 `.fgopetpack`、外部 SHA-256 和变更说明；Phase 1 应用不调用 GitHub API。用户下载后可双击文件或从从者库选择文件安装。
...
恢复顺序为当前有效版本、同包上一有效版本、最后验证成功的包、内置玛修包。内置包也无效时显示可诊断的启动错误并安全退出。
...
建立 solution、依赖注入、配置和日志；迁移画像 schema、loader、统一几何与 WPF 双层画像；内置玛修包在无外部运行时条件下启动；移植 Phase 0 测试。

### P1-B 从者扩展包系统

实现 pack schema v1、art schema v3、v2 到 v3 迁移 fixture、安装事务、版本和回退、从者库与设置窗口、从者/外观切换；把玛修转换为不可卸载的首发内置包。
...
- 无 Python、LLM、Codex 插件或用户包时，应用使用内置玛修包正常启动。
